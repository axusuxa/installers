...
fqdn=`/bin/hostname`
publicip=`wget -qO- http://ip.axus.pw`
...
git clone https://github.com/axusuxa/zpanelx.git
cd zpanelx/
...
cp -R /etc/zpanel/panel/etc/build/config_packs/ubuntu_16_04/. /etc/zpanel/configs/
...
mysql -u root -p$password -e "UPDATE mysql.user SET authentication_string=PASSWORD('$postfixpassword') WHERE User='postfix' AND Host='localhost';";
mysql -u root -p$password -e "FLUSH PRIVILEGES";
...
sed -i "s|#SQLConnectInfo  zpanel_proftpd@localhost root password_here|SQLConnectInfo   zpanel_proftpd@localhost zroot $password|" /etc/zpanel/configs/proftpd/proftpd-mysql.conf
...
sed -i "s|;upload_tmp_dir =|upload_tmp_dir = /var/zpanel/temp/|" /etc/php/7.0/cli/php.ini
sed -i "s|;upload_tmp_dir =|upload_tmp_dir = /var/zpanel/temp/|" /etc/php/7.0/apache2/php.ini
